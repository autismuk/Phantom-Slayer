...
function PlayerManager:onMessage(sender,body)
	...
	if body.name == "nextstate" then 															-- message to go to next state
		Game:event("gameover", { score = self:getExecutive().e.score:getScore() })
	end
...
function Score:constructor()
	self.m_scoreText = display.newBitmapText("0000",5,display.contentHeight-5,"dub",				-- Score text
...
--//	Return score + anything outstanding
--//	@return 	[number]		Score 

function Score:getScore()
	return self.m_score + self.m_toAdd
end 
