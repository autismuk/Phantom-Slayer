...
function ScreenObject:constructor(info) 
	...
	self.m_object.x = info.rectangle[1] + self.m_object.width / 2
	self.m_object.y = info.rectangle[2] + self.m_object.height / 2
...
function EndGameFactory:preOpen(info,eData)
	...
	ScreenObject:new(self:getExecutive(),{ object =  display.newBitmapText("",0,0,"dub",52),rectangle = { 50,25 }, data = e1 }):makeClickable("counter",nil)
	ScreenObject:new(self:getExecutive(),{ object =  display.newBitmapText("",0,0,"dub",52),rectangle = { 50,35 }, data = e2 } ):makeClickable("playnow",nil)
	StateChangeObject:new(self:getExecutive(), { object = "images/maze.png", rectangle = { 70,70,30,30 }}):makeClickable("restart")
end 
